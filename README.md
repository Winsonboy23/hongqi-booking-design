...
`index.html` 是給業主看的設計稿畫廊：五個分頁（介面設計new／系統流程圖／前台手機／前台桌機／後台），縮圖是實際運作中的畫板，點開可以放大操作，支援 ← → 換頁與 Esc 關閉。畫板裡連到其他畫板的按鈕會直接換到那張；介面設計new 的卡片與燈箱有色塊可以切換 Tweaks 主色。畫廊外框的樣式照簡約 A・清爽（白底、1px 細線、8px 圓角、無陰影、思源黑體），文字一律黑色，畫板本身不受影響。
...
### 簡約風格提案・手機 390（2026-09-22，畫廊分頁「介面設計new」）
