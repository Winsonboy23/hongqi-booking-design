...
`index.html` 是給業主看的設計稿畫廊：兩個分頁——「介面設計」放課程資訊頁的新提案 A、B 與舊版，「介面流程」放前台手機 5 張。系統流程圖、前台桌機、後台目前先隱藏，要放回來把 `index.html` 裡那一組的 `hidden: true` 拿掉即可。縮圖是實際運作中的畫板，點開可以放大操作，支援 ← → 換頁與 Esc 關閉。畫板裡連到其他畫板的按鈕會直接換到那張；A、B 的卡片與燈箱有色塊可以切換 Tweaks 主色。畫廊外框的樣式照簡約 A・清爽（白底、1px 細線、8px 圓角、無陰影、思源黑體），文字一律黑色，畫板本身不受影響。
